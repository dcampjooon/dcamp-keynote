...
export async function PUT(request: Request, ctx: RouteContext<"/api/templates/[id]">) {
  ...
  const { data, error } = await supabaseAdmin
    .from("templates")
    .update({ name: b.name?.trim(), description: b.desc ?? "", theme: { tokens: b.tokens ?? {}, surround: b.surround ?? "#0a0e24", swatch: b.swatch ?? ["#2f6df6", "#14b8c4"], layouts: b.layouts ?? [], pdfPath: b.pdfPath ?? "", page: b.page ?? { size: "16:9", orientation: "landscape" } } })
    .eq("id", id)
